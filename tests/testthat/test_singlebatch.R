...
test_that("test_marginal_hard", {
    ...
    model <- posteriorSimulation(model)
    ## restart at modal values. label switching should not occur
    mcmcParams(model) <- McmcParams(iter=500, nStarts=0)
    model <- posteriorSimulation(model)
    i <- order(theta(model))
...
test_that("test_marginal_pooled2", {
    ...
    ##trace(CNPBayes:::posteriorSimulationPooled, browser)
...
test_that("test_selectK_easy", {
    ...
    if(FALSE){
      ...
      plist4 <- ggSingleBatchChains(mod4)
      plist4[["comp"]]
...
test_that("posteriorSimulation methods", {
    ...
    # multiple components
    post <- posteriorSimulation(model, k=1:3)
...
test_that("targeted_seq data", {
  ...
  ##
  set.seed(123)
  mp <- McmcParams(iter=500, burnin=0, nStarts=20)
