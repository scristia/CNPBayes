...
deviation for each component, and a *batch* model with one mean and
standard deviation for each batch of each component. Estimation of the 
posterior is by Markov Chain Monte Carlo (MCMC) written in C++ using the Rcpp 
package [@Rcpp].
...
this model by some of the same authors was developed to fully automate
the analysis of the Welcome Trust Case Control Consortium (WTCCC)
genotype data [@cardin] and implemented in the R package CNVCALL
...
Batch effect refers to the differences between groups of samples due to
technical reasons and can vary by CNP locus. This variation can be due to 
differences in date, temperature, or lab technician [@batch-effect]. In order
to model batch effect, copy number must be estimated by CNP locus.
Since batch is unknown *a priori*, we use chemistry plates
as a surrogate, since samples on a chemistry plate are processed at the same,
and are run on the same PCR. It is not computationally feasible to fit a model
that is hierarchical over all plates. However not all plates are from
different distributions. In order to identify these plates with no differences,
`collapseBatch` is included in CNPBayes. `collapseBatch` uses the
Kolmogorov-Smirnov test to identify plates that give rise to the same
distributions.
