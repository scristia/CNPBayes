...
integrated over. While marginal likelihood is computed and used for determing
*k*, the number of latent variable, Chib's Estimator and a variant of Chib's
Estimator created by Berkhof et al is also calculated. The log Bayes Factor is
computed comparing the two models with the highest marginal likelihood.
...
is supported. We can also show that there is no underfitting by merging so show
using merge with correct three component model.
...
Need to also include plot of the batch model to show multiple densities for
single component. A lot of the work for merging is in CNVTools. Can capture
