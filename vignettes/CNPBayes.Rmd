...
- `eta.0` and `m2.0` priors for 
  $\tau^2 \sim \text{Ga(rate=`eta.0`, shape=`m2.0`)}$, the overall variance 
...
- `a` and `b` priors for $\sigma^2_0 \sim \text{Ga(shape=`a`, rate=`b`)}$, the 
...
hypp <- Hyperparameters(type="batch", k=3, eta.0=1, m2.0=0.3)
...
## Create Hyperparameters for batch model
hypp <- Hyperparameters(type="batch", k=3, eta.0=600, 
                        m2.0=(600 ^ 2) / 194400000)
