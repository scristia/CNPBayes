...
standard deviation for each batch of each component. Approximation of
the posterior is by Markov Chain Monte Carlo (MCMC) written in C++
using the Rcpp package [@Rcpp].
...
estimate the number of components *K*, the `computeMarginalLik`
function is used.
