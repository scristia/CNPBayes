...
model2 <- MarginalModel(data=y(sim.data), k=2, mcmc.params=mp)
model3 <- MarginalModel(data=y(sim.data), k=3, mcmc.params=mp)
mlist <- list(posteriorSimulation(model2),
              posteriorSimulation(model3))
x <- computeMarginalLik2(mlist)
orderModels2(x)
```
...
consuming, especially for batch data. For each number of components, the
...
JC: Caution for cmpring nontested models.
