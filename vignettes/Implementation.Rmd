...
- `eta.0` and `m2.0` priors for $\tau^2 \sim$ Ga(shape=`eta.0`, rate=`m2.0`),
  the overall variance across components. (For the `BatchModel` discussed in
  greater detail below, $\tau^2$ is component-specific, representing the
  cross-batch variance of means for each component.)
...
To visualize the data and overlay the model-based approximation of the posterior:
...
similar to that of a `MarginalModel`. The `BatchModel` is hierarchical over the
batches, and thus requires information about the batches. Instances of
`McmcParams` are equivalent between `BatchModel` and `MarginalModel`. However,
an object of class `Hyperparameters` is constructed with `type = "batch"`.
Additionally, simulated batch data is created using `simulateBatchData` which
requires `theta` and `sds` to be specifed as $B \times K$ matrices, for $K$
components and $B$ batches. `simulateBatchData` also requires a `batch`
parameter, labelling the batch of each observation. Finally, `BatchModel`s are
constructed using the function `BatchModel` which operates similarly to
`MarginalModel` but requires a `batch` parameter, similar to `Hyperparameters`.
`plot`s of `BatchModel`s include batch specific density estimates.

```{r batch}
## Create McmcParams for batch model
mp <- McmcParams(iter=500, burnin=50, thin=1, nStarts=10)
...
Generally, the number of components is not known *a priori*. To estimate the
number of components *K*, we estimate the marginal likelihood using the
`marginalLikelihood` function. It is important to note that a list of models
passed to this function must have converged.

```{r marginalLikelihood}
mp <- McmcParams(iter=500, burnin=100, nStarts=10)
mlist <- MarginalModelList(data=y(sim.data), k=2:3, mcmc.params=mp)
...
```{r bic}
## simulate k=2 model
sim.data <- simulateData(N=500, p=rep(1/3, 3),
                         theta=c(-1, 0, 1),
                         sds=rep(0.1, 3))
...
Finally, the mixture components may not necessarily correspond to distinct copy
number states. For example, if the number of probes in a CNP region is small,
the average log R ratio calculated for each sample is more likely to be skewed.
In practice, one would fit many models (e.g., k = 1 - 4), select the best model
by the marginal likelihood, and then map components from the best model to
distinct copy number states. However, to illustrate the mapping procedure we
intentionally overfit the 3-component model in the code chunk below by setting
`k=4`. We then instantiate a copy number model (`SingleBatchCopyNumber`) and
apply the function `mapComponents`. Note that posterior probabilities for copy
number states take into account the many-to-one mapping of components to copy
number and that the plotting function appropriately color codes the mixtures by
the copy number state.

```{r mapComponents}
mp <- McmcParams(iter=100, burnin=100, thin=1, nStarts=10)
...
sim.data <- simulateBatchData(N=N,
                              ...
                              sds=sds,
                              p=c(1/5, 1/3, 1-1/3-1/5))
ggMultiBatch(sim.data)
model <- BatchModel(data=y(sim.data), k=3,
                    batch=collapseBatch(sim.data))
ggMultiBatch(model)
```
