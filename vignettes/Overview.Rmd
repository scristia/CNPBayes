...
mp <- McmcParams(nStarts=50, burnin=0, iter=1000)
model.marginal <- MarginalModel(data=med.summary, mcmc.params=mp)
...
model (not evaluated) have the same interface.

```{r posteriorSimulation}
set.seed(1337)
mlist.sb <- posteriorSimulation(model.marginal, k=1:4)
mcmcParams(mlist.sb) <- McmcParams(iter=1000, burnin=0, nStarts=0)
mlist.sb <- posteriorSimulation(mlist.sb)
##mlist.mb <- posteriorSimulation(model.batch, k=1:4)
##mlist.mb <- posteriorSimulation(mlist.mb)
```
...
sb.lik <- marginalLikelihood(mlist.sb)
```
...
component.
```{r map}
select <- which.max(sb.lik)
selected.model <- mlist.sb[[select]]
map(selected.model)
```
