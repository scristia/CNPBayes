...
mp <- McmcParams(iter=50, burnin=10, thin=1, nStarts=10)
model <- MarginalModel(data=lrr, mcmc.params=mp)
m.list <- posteriorSimulation(model, k=1:4)
## run 10 additional iterations using the last iteration of the above chain as
## the starting value
mcmcParams(m.list) <- McmcParams(nStarts=0, iter=100, burnin=0)
m.list <- posteriorSimulation(m.list)
```
