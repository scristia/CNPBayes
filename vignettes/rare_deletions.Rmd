...
Here, we assume that the batch surrogates have already been identified and are as shown in Figure 1. See [ other vignette ] for identification of batch surrogates.  The next step is to assess whether the region is a  deletion polymorphism by evaluating whether any of the one dimensional summaries (here, average log R ratios) are consistent with a homozygous deletion.  Below, we use an ad-hoc approach that simply evaluates whether there are multiple samples with average log R ratios less than some threshold. 
...
The mixture components plotted in Fig 2 do not correspond to the same copy number state, complicating downstream inference.  This can also be detected by inspecting the current values of $\theta$ and $\sigma$.
...
Programmatically, we could compare the fold change of the standard deviations of the mixture component.  A fold change larger than 5 would be suprising.  In addition, we can check whether any of the first component means would be consistent with the observations that are likely homozygously deleted:
...
## Two strategies for dealing with unobserved homozygous deletions in a subset of batches
...
While the standard deviations of the mixture components may be inflated, the SB3 model does fairly well and we can see that the programmatic checks for the deletion model are more reasonable.
...
```{r mb2}
mbl2 <- filter(assays(mb), oned > THR) %>%
  MultiBatchList(data=.)
mb2 <- mbl2[["MB2"]]
iter(mb2) <- iter(mb3)
burnin(mb2) <- burnin(mb3)
...
We now have reasonable starting values for components two and three across the multiple batches and a marginal mean for homozygous deletions from model SB2.  We use this information to impute homozygous deletions for the batches in which no homozygous deletion was observed.
...
Given the above `MultiBatchList`, we now restrict the model space to those with at least 3 components.  Below, we fit a small number of burnin iterations and keep the models with the highest log likelihood.
...
pdf("mixmodel_comparison.pdf", width=10, height=12)
grid.newpage()
...
```
\begin{figure}
\includegraphics[width=0.8\textwidth]{mixmodel_comparison.pdf}
